...
lr_schedular_G = torch.optim.lr_scheduler.LambdaLR(
...
# Buffers of previously generate samples
...
def sample_images(batches_done):
    ...
    save_image(image_grid, f'images/{opt.dataset_name}/{batches_done}.png', normalize=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        loss_id_B = criterion_identity(G_AB(real_A), real_A)
        ...
        # Print Log
        sys.stdout.write(
            f"\r[Epoch {epoch}/{opt.n_epochs}] [Batch {i}/{len(dataloader)}] [D loss: {loss_D.item()}] [G loss: {loss_G.item()}, adv: {loss_GAN.item()}, cycle: {loss_cycle.item()}, identity: {loss_identity.item()}] ETA {time_left}"
        )
